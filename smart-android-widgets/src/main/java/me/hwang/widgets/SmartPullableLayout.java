...
public class SmartPullableLayout extends ViewGroup implements NestedScrollingParent
{
    ...
    public void setPageIn(int pageIn)
    {
        ...
        if(pageIn >= pageCount)
        {
            tvHintPullUp.setText(R.string.smart_ui_pull_up_done);
        }else
        {
     ...
     * @param pageIn
     */
    public void updatePage(int pageCount,int pageIn)
    {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom)
    {
        for (int i = 0; i < getChildCount(); i++)
        {
            ...
            if (child == mPullableHeader)
            { // 头视图隐藏在顶端
                child.layout(0, 0 - child.getMeasuredHeight(), child.getMeasuredWidth(), 0);
            } else if (child == mPullableFooter)
            { // 尾视图隐藏在末端
    ...
    {
        if (getScrollY() != 0)
        { // 只有在SmartPullableLayout自身已经发生过位移的情况才处理预消耗
